...
- **digest 写权限过宽**（双签 high）：digest+cwd 仍带 `--dangerously-skip-permissions`。需先实测 agy `--sandbox` 在 `-p` 下是否可用（不挂起）；可用则 digest dir-scan 改 `--sandbox`，否则 digest 默认 embed-only。当前 dir-scan 路径=已知风险。
