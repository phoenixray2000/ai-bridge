...
4. **Gemini 模型动态发现**：按家族关键词+档位后缀匹配（版本号不参与），清单缓存落盘（带 agy 版本+时间戳）。**刷新是失败驱动，不定时不手动**：匹配未命中 → 自动重跑 `agy models` 刷新再试一次 → 仍未命中才 fail loud 并列出可用名（agy 对无效模型名的行为未知，不能依赖它报错）；辅助触发：bridge 启动时 agy 版本号与缓存不符则顺手刷新。env override 仍是人工旁路。
